...
var reverse = function (string) {
    ...
    result += string.split('').pop();
    result += reverse(string.substring(0, string.length - 1));
    return result;
};
...
var multiply = function (x, y) {
    ...
    if (y < 0 && x > 0) {
        y = -y;
        var ans = x + multiply(x, y - 1);
        return -ans;
    }
    if (x < 0 && y > 0) {
        var ans1 = (-x + multiply(-x, y - 1));
        return -ans1;
    }
    if (y < 0 && x < 0) {
        x = -x;
        y = -y;
        var ans2 = x + multiply(x, y - 1);
        return ans2;
    }
    if (x > 0 && y > 0) {
...
var countOccurrence = function (array, value) {
    ...
    array = array.slice(1);
    result = result + countOccurrence(array, value);
    return result;
};
...
var alternateSign = function (array) {
    ...
    var curr = Math.abs(array[0]);
    var next = Math.abs(array[1]);
    var result = alternateSign(array.slice(2));
    result.unshift(curr, -next);
    return result;
};
...
var numToText = function (str) {
    ...
    var temp = numToText(str.substring(0, lastIndex));
    var replace;
    switch (lastChar) {
        case '1': replace = 'one';
            break;
